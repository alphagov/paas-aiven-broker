...
var _ = Describe("OpenSearch Client", func() {
	var (
		client *Client
	)
	...
	Context("making requests", func() {
		...
		It("should be able to get Version() from OpenSearch", func() {
			httpmock.RegisterResponder("GET", "http://localhost:9200",
				httpmock.NewStringResponder(200, `{"version":{"number":"5.0.0"}}`))
		...
		It("should fail to get Version() due to OpenSearch not providing one", func() {
			httpmock.RegisterResponder("GET", "http://localhost:9200",
